...
function add_async($tag, $handle)
{
    ...
    is_single() ? $jsonLD .= '[' . json_encode(set_bread_json()) . ',' . json_encode(set_single_json()) . ']' : $jsonLD .= json_encode(set_bread_json());
    $jsonLD .= '</script>';
    $replace = '<script> var path ="' . get_template_directory_uri() . '";</script>' . $tag .$jsonLD;
